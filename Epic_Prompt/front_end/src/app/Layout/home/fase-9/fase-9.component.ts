...
@Component({
  selector: 'app-fase-9',
  templateUrl: './fase-9.component.html',
  styleUrl: './fase-9.component.css'
})
export class Fase9Component  implements OnInit{
  ...
  carregaVilao(){
    this.vilaoController.getVilao(9).subscribe(res => {
      this.vilao = res;
  ...
  vilaoRelizaAcao(){
    if(this.vidaAtualVilao <= this.vilao.vida * 0.35 && this.curaProfunda == 0){
      ...
      if (randomNumber === 0 || this.curaProfunda > 0) {
        //vilão ataca
        return 0;
      } else {
        //vilão defende
